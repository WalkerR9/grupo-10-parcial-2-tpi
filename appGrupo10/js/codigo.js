...
function guardar2(productos) {
	...
	for (i = 0; i < productos.length; i++) {
		...
		var categoria = productos[i].categoria;
		var titulo = productos[i].titulo;
		var fila = "<td>" + id + "</td><td>$" + precio + "</td><td>" + titulo + "</td><td>" + categoria + "</td> <td><a href= \" " + productos[i].id + "\"class=\"btn btn-warning\">Actualizar</a><a class=\"btn btn-danger\" onclick=\"ELiminar(" + productos[i].id + ")\">Eliminar</a></td>";
